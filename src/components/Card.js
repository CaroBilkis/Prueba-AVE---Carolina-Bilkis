...
const Card = (props) => {
  ...
  return (
    <li  key={props.id} className="flex flex-col aspect-[1/1] justify-between items-center h-full flex-shrink-0 gap-4 pt-10 pb-5 px-4 drop-shadow-xl overflow-hidden rounded-3xl bg-slate-50 hover:drop-shadow-2xl group ">
      ...
      <div className='h-2/3 p-4 self-center aspect-auto mix-blend-multiply   '>
    ...
    <button className='absolute right-6 top-6' onClick={()=>controlLike()}>
            
            <svg className={like ? 'text-red-500 scale-110 animation-300 w-8 h-8' : 'opacity-30 text-gray-400 scale-100 animation-300  w-8 h-8'} width="24" height="24" viewBox="0 0 24 24" fill="black" xmlns="http://www.w3.org/2000/svg">
